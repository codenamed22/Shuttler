    ...
    const interval = setInterval(() => {
      setBus(prevBus => {
        ...
        if (prevBus.status === 'active') {
          const newLat = prevBus.currentLocation[0] + (Math.random() - 0.5) * 0.001;
          const newLng = prevBus.currentLocation[1] + (Math.random() - 0.5) * 0.001;
      ...
      <div className="bg-white rounded-lg shadow-sm border p-6">
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900 mb-1">{bus.name}</h1>
              <p className="text-gray-600">{bus.origin} → {bus.destination}</p>
          ...
          <div className="text-right">
            <div className={clsx(
              ...
              {bus.status === 'active' && <div className="w-2 h-2 bg-white rounded-full mr-2 animate-pulse" />}
              {currentStatus.text}
            </div>
            <p className="text-xs text-gray-500">{currentStatus.description}</p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-gray-50 rounded-lg p-4">
            ...
            <div className="text-2xl font-bold text-gray-900">{bus.occupancy}/{bus.capacity}</div>
          ...
          <div className="bg-gray-50 rounded-lg p-4">
            ...
            <div className="text-2xl font-bold text-gray-900">{completedStops}/{bus.stops.length}</div>
          ...
          <div className="bg-gray-50 rounded-lg p-4">
            ...
            <div className="text-sm font-mono text-gray-900">
              {bus.currentLocation[0].toFixed(4)}, {bus.currentLocation[1].toFixed(4)}
            </div>
        ...
        {bus.status === 'delayed' && (
          <div className="mt-4 bg-warning-50 border border-warning-200 rounded-lg p-4">
            ...
            <p className="text-warning-700 mt-1">
              This bus is currently running behind schedule due to traffic conditions. 
              Estimated delay: 10-15 minutes.
            </p>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2">
          <div className="bg-white rounded-lg shadow-sm border p-4">
            <h2 className="text-lg font-semibold text-gray-900 mb-4">Live Route Map</h2>
            <MapView bus={bus} />
